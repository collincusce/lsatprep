...
import { pickQuestions, pickQuestionsWithDifficultyMix, seededRng } from '../selector.js';
...
function assembleTest(bank, prefs) {
  ...
  const lr1 = pickQuestionsWithDifficultyMix({ bank, filters: { section: 'LR' }, count: 25, seenIds, rng });
  const lr2 = pickQuestionsWithDifficultyMix({ bank, filters: { section: 'LR' }, count: 25, seenIds, rng });
  const rc  = pickQuestionsWithDifficultyMix({ bank, filters: { section: 'RC' }, count: 27, seenIds, rng });
  const fourth = prefs.lgEnabled
    ? pickQuestionsWithDifficultyMix({ bank, filters: { section: 'LG', lgEnabled: true }, count: 23, seenIds, rng })
    : pickQuestionsWithDifficultyMix({ bank, filters: { section: 'LR' }, count: 23, seenIds, rng });
